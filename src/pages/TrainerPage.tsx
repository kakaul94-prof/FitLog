...
import { Brain, ChevronLeft, Plus, Send, Square } from 'lucide-react'
...
// Ask a trainer: a chat about YOUR training. Every question ships with a
// snapshot of the user's own logged lifts (lib/trainerContext.ts) plus their
// saved memory, so answers cite real weights and dates instead of giving
// generic advice. The model call happens server-side in functions/api/trainer.ts.
...
export function TrainerPage() {
  ...
  // The training snapshot is expensive-ish (a handful of queries) and stable
  // for the life of the chat, so it's built once on the first question.
  ...
  // Abandon an in-flight answer if the page is left.
  ...
  const send = async (question: string) => {
    ...
    try {
      if (contextRef.current == null) {
        try {
          contextRef.current = await buildTrainerContext()
        } catch {
      ...
      if (controller.signal.aborted) {
        ...
        setBubbles((b) =>
          b[b.length - 1]?.content.trim() ? b : b.slice(0, -1),
        )
      } else {
        patch({
  ...
  const onRemember = (fact: string) => {
    addFact(fact)
    setSaved(fact)
    setBubbles((b) => b.map((x) => (x.remember === fact ? { ...x, remember: null } : x)))
  }
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
        {bubbles.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-center">
            ...
            <h2 className="text-lg font-semibold">Ask about your training</h2>
            <p className="mt-1 max-w-[17rem] text-sm text-muted-foreground">
              I can see your logged lifts, program and goals. Ask about aches,
              plateaus, or what to do next.
            </p>
