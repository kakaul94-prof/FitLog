...
/** Score each tracked nutrient's per-logged-day average against its DV. Pure,
 *  so the Week/Month in review can run it over any date range's diary rows. */
export function analyzeMicros(rows: MicroRow[], dailyMicros: Nutrients = {}): MicroTrends {
...
export function useMicronutrientTrends(days: number, dailyMicros: Nutrients = {}) {
  return useQuery({
    ...
    queryFn: async (): Promise<MicroTrends> => {
      ...
      return analyzeMicros((data ?? []) as MicroRow[], dailyMicros)
    },
  })
}
